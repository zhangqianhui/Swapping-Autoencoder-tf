...
def get_gan_losses_fn():
    bce = functools.partial(tf.nn.sigmoid_cross_entropy_with_logits)
    def d_loss_fn(r_logit, f_logit):
        r_loss = bce(labels=tf.ones_like(r_logit), logits=r_logit)
        f_loss = bce(labels=tf.zeros_like(f_logit), logits=f_logit)
        return tf.reduce_mean(r_loss + f_loss)

    def g_loss_fn(f_logit):
        f_loss = bce(labels=tf.ones_like(f_logit), logits=f_logit)
